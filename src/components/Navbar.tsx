...
export default function Navbar({ onConsultationClick, onAdminClick }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('mahizham_products_updated', handleProductsUpdate);
  }, []);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('mahizham_add_to_cart_triggered', handleAddEvent);
  }, []);
  ...
  const subtotal = resolvedCartItems.reduce((acc, curr) => acc + curr.totalItemPrice, 0);
  ...
  const navLinks = [
    { name: 'Botanicals', href: '#products-catalog' },
                  ...
                  <div className="flex-grow flex flex-col justify-between">
                      ...
                      {resolvedCartItems.map((item, idx) => (
                        <div key={`${item.productId}-${item.size}`} className="flex gap-4 items-center">
                          ...
                          <div className="flex-grow min-w-0">
                            <h4 className="font-display text-sm font-bold text-primary truncate">{item.name}</h4>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <div className="flex items-center border border-outline-variant/30 rounded-lg bg-surface-container-low">
                                ...
                                <span className="px-2 font-body text-xs font-bold text-primary">{item.quantity}</span>
                                <button 
                                  onClick={() => updateQuantity(item.productId, item.size, 1)}
                                  className="px-2.5 py-1 text-primary hover:text-secondary transition-colors"
                                >
                    ...
                    <div className="border-t border-outline-variant/30 pt-6 mt-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="font-display text-2xl font-bold text-primary">₹{subtotal}.00</span>
                      </div>
                      <button 
                ...
                <div className="flex-grow flex flex-col justify-between">
                  <form onSubmit={handlePlaceOrderSubmit} className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="block font-body text-[10px] font-bold text-primary uppercase">Full Name</label>
                      <input
                        type="text"
                        value={custName}
                    ...
                    <div className="space-y-1.5">
                      <label className="block font-body text-[10px] font-bold text-primary uppercase">Phone Number</label>
                      <input
                        type="tel"
                        value={custPhone}
                    ...
                    <div className="space-y-1.5">
                      <label className="block font-body text-[10px] font-bold text-primary uppercase">Email Address</label>
                      <input
                        type="email"
                        value={custEmail}
                    ...
                    <div className="space-y-1.5">
                      <label className="block font-body text-[10px] font-bold text-primary uppercase">Delivery Address</label>
                      <textarea
                        value={custAddress}
              ...
              {checkoutStep === 'success' && (
                <div className="flex-grow flex flex-col justify-center items-center text-center space-y-6 py-10">
                  ...
                  <div className="space-y-2">
                    <h4 className="font-display text-xl text-primary font-bold">Order Received!</h4>
                  ...
                  <div className="bg-[#FAF9F5] border border-outline-variant/35 p-4 rounded-2xl w-full text-center space-y-1">
                    <span className="block font-body text-[10px] text-on-surface-variant uppercase font-semibold">Your Order ID</span>
                    <span className="block font-display text-lg font-extrabold text-primary tracking-wider">{placedOrderId}</span>
                  </div>
                  <button
                    onClick={() => setIsCartOpen(false)}
                    className="w-full bg-primary hover:bg-primary-container text-on-primary font-body text-xs font-semibold tracking-widest uppercase py-4 rounded-full transition-colors cursor-pointer"
